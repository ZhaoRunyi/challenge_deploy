...
def bool_attr(container: Any, name: str, default: bool = False) -> bool:
    return bool(getattr(container, name, default))


def joint_values_from_sdk(joint_state: Any) -> np.ndarray:
...
class SinglePiperArm:
    ...
    def require_motion_allowed(self, action: str) -> None:
    ...
    def enable(self, *, retries: int = 5, sleep_s: float = 0.5) -> bool:
        self.require_motion_allowed("enable the arm")
    ...
    def set_joint_mode(self, *, speed_percent: int = 100) -> None:
        self.require_motion_allowed("switch to joint control mode")
    ...
    def exit_teach_mode(self) -> None:
        self.require_motion_allowed("exit teach mode")
    ...
    def read_state(self, *, prefer_joint_ctrl: bool = False) -> PiperArmState:
        ...
        joint_feedback_timestamp_s = float(getattr(joint_feedback_msgs, "time_stamp", 0.0))
        joint_command_timestamp_s = float(getattr(joint_ctrl_msgs, "time_stamp", 0.0))
        gripper_feedback_timestamp_s = float(getattr(gripper_feedback_msgs, "time_stamp", 0.0))
        gripper_command_timestamp_s = float(getattr(gripper_ctrl_msgs, "time_stamp", 0.0))
        end_pose_timestamp_s = float(getattr(end_pose_msgs, "time_stamp", 0.0))
        high_spd_timestamp_s = float(getattr(high_spd, "time_stamp", 0.0))
        status_timestamp_s = float(getattr(status_msgs, "time_stamp", 0.0))

        qpos_feedback = np.concatenate(
            (
                joint_values_from_sdk(joint_feedback_msgs.joint_state),
        ...
        qpos_command = np.concatenate(
            (
                joint_values_from_sdk(joint_ctrl_msgs.joint_ctrl),
        ...
        qvel = np.array(
            [
                motor_field(high_spd, 1).motor_speed / 1000.0,
                motor_field(high_spd, 2).motor_speed / 1000.0,
                motor_field(high_spd, 3).motor_speed / 1000.0,
                motor_field(high_spd, 4).motor_speed / 1000.0,
                motor_field(high_spd, 5).motor_speed / 1000.0,
                motor_field(high_spd, 6).motor_speed / 1000.0,
                0.0,
            ],
            dtype=np.float64,
        )
        ...
        return PiperArmState(
            ...
            status=status_to_dict(status_msgs.arm_status),
    ...
    def move_to_joint_positions(
        ...
        *,
        ...
        self.require_motion_allowed("move the arm")
