...
def new_session_id() -> str:
    return f"dreamzero_{time.strftime('%Y%m%d_%H%M%S')}"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="DreamZero SLAI Piper client: capture snapshots, infer chunks, and command Piper in a rollout."
    )
    parser.add_argument(
        "--train-config",
        required=True,
        help="DreamZero YAML config path, e.g. examples/sft/config/piper_sft_dreamzero_5b.yaml.",
    )
    ...
    parser.add_argument("--num-inference-timesteps", type=int, default=None, help="Forward an optional denoising step override to the server.")
    ...
    parser.add_argument("--record-dir", default=str(DEPLOY_ROOT / "artifacts" / "dreamzero_records"))
...
def run_once(args: argparse.Namespace) -> None:
    ...
    if resolved_prompt is None:
        raise RuntimeError(
            "No prompt available. Provide --prompt, or ensure the remote DreamZero server was started with --default_prompt."
        )
    print(
        json.dumps(
            {
